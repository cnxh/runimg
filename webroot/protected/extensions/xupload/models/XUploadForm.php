...
class XUploadForm extends CFormModel
{
        ...
        */
        ...
        private $fileSizeLimit = 1048576;//1M
        private $fileExtensionLimit = array('image/gif', 'image/jpeg', 'image/png', 'image/x-png');
        ...
        public function rules()
        {
                return array(
                        array('file', 'file'),
                        array('size', 'checkSize'),
                        array('mine_type', 'checkMineType')
                );
        }
    ...
    public function checkSize(){
        if(!$this->hasErrors()){
            if($this->size > $this->fileSizeLimit)
                $this->addError('file', '文件大小不能超过1M');
        }
    }


    public function checkMineType(){
        if(!$this->hasErrors()){
            if(!in_array($this->mime_type, $this->fileExtensionLimit))
                $this->addError('file', '文件格式暂只支持jpg/jpeg/gif/png');
        }
    }
